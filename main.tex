\documentclass[a4paper,12pt]{article}

\input{etc/preamble}

\usepackage{xcolor}

\newtheorem{theorem}{Теорема}[section]
\newtheorem{lemma}{Лемма}[section]
\newtheorem{definition}{Определение}[section]
\newtheorem*{note}{Замечание}

\counterwithin{equation}{section}
\counterwithin{figure}{section}

\renewcommand{\b}{\mathbf}
\renewcommand{\div}{\, \text{div} \,}
\newcommand{\rot}{\, \text{rot} \,}

\title{Билеты к ГКЭ по общей физике}
\author{Николай Чусовитин}
\date{2021-2022 учебный год}

\begin{document}

\maketitle

\section*{Предисловие}

Данный документ содержит в себе материалы для подготовки к ГКЭ по общей физике на основе экзаменационной программы 2021-2022 года. При составлении использовались в основном учебники Д.В. Сивухина и Н.А. Кириченко. Раздел, посвящённый квантовой физике, написан в основном с использованием лекционного курса В. Н. Глазкова.

Поскольку документ составлялся в течение длительного времени с большими перерывами, стиль и подробность повествования и оформления могут меняться от части к части. Тем не менее, он содержит теоретический минимум по каждому вопросу программы, а зачастую чуть больше, чем минимум.

For aerospace bot squadron with love.

\newpage

\tableofcontents

\newpage
\part{Механика}

\input{questions/1.tex}
\newpage
\input{questions/2.tex}
\newpage
\input{questions/3.tex}
\newpage
\input{questions/4.tex}
\newpage
\input{questions/5.tex}
\newpage
\input{questions/6.tex}
\newpage
\input{questions/7.tex}
\newpage
\input{questions/8.tex}
\newpage
\input{questions/9.tex}
\newpage

\part{Термодинамика и молекулярная физика}

\input{questions/10.tex}
\newpage
\input{questions/11.tex}
\newpage
\input{questions/12.tex}
\newpage
\input{questions/13.tex}
\newpage
\input{questions/14.tex}
\newpage
\input{questions/15.tex}
\newpage
\input{questions/16.tex}
\newpage
\input{questions/17.tex}
\newpage
\input{questions/18.tex}
\newpage
\input{questions/19.tex}
\newpage
\input{questions/20.tex}
\newpage

\part{Электричество и магнетизм}

\input{questions/21.tex}
\newpage
\input{questions/22.tex}
\newpage
\input{questions/23.tex}
\newpage
\input{questions/24.tex}
\newpage
\input{questions/25.tex}
\newpage
\input{questions/26.tex}
\newpage
\input{questions/27.tex}
\newpage
\input{questions/28.tex}
\newpage
\input{questions/29.tex}
\newpage
\input{questions/30.tex}
\newpage
\input{questions/31.tex}
\newpage
\input{questions/32.tex}
\newpage
\input{questions/33.tex}
\newpage
\input{questions/34.tex}
\newpage

\part{Оптика}

\textit{Под интенсивностью света в этом разделе мы будем понимать величину $I = \overline{E^2}$, если не оговорено противное}.
\input{questions/35.tex}
\newpage
\input{questions/36.tex}
\newpage
\input{questions/37.tex}
\newpage
\input{questions/38.tex}
\newpage
\input{questions/39.tex}
\newpage
\input{questions/40.tex}

\end{document}